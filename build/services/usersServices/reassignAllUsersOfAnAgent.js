...
const reassignAllProspectsOfAnAgent = async (request, response) => {
    try {
        ...
        if (request.query.newAgentId) {
            ...
            const newAgentCheck = (await agentEntity_1.default.findOne({ where: { id: agentId } }));
            const oldAgentCheck = (await agentEntity_1.default.findOne({ where: { id: oldAgentId } }));
        ...
        await agentEntity_1.default.update({ no_of_prospects: 0 }, { where: { id: oldAgentId } });
        const newAgentCheck = (await agentEntity_1.default.findOne({ where: { id: agentWithLowestProspects.id } }));
        const oldAgentCheck = (await agentEntity_1.default.findOne({ where: { id: oldAgentId } }));
        return response.status(201).json({
            status: `success`,
