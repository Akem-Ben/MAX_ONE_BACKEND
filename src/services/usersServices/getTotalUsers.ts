...
export const getTotalUsersByStages = async (request: Request, response: Response) => {
    try {
      ...
      stages.forEach((stage: string) => {
        const stageUsers: UserAttributes[] = users.filter((user:UserAttributes) => user.stage === stage);
