...
export const reassignAllProspectsOfAnAgent = async(request:Request, response:Response) => {
    try{
        ...
        if(request.query.newAgentId){
            ...
            await Users.update({agent_id:agentId}, {where:{agent_id:oldAgentId}}) as unknown as UserAttributes
            
            const oldAgent = await Agent.findOne({where: {id:oldAgentId}}) as unknown as AgentAttributes
        ...
        const agentWithLowestProspects = await Agent.findOne({
          ...
          await Users.update({agent_id:new_agent_id}, {where:{agent_id:oldAgentId}}) as unknown as UserAttributes
